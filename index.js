...
var base = require("core");
     ...
     *  var queryObj = base.mapQuery(url);
     * };
     */
    mapQuery = function(uri){
        ...
        for(i=0; i<pieces.length; i++){
            try{
                ...
                //base.out("错误：[" + e.name + "] "+e.message+", " + e.fileName+", 行号:"+e.lineNumber+"; stack:"+typeof e.stack, 2);
            }
        }
        return params;
    };
    ...
    stripScripts = function(string, option){
        ...
        if (option === true){
            eval(scripts);
        }else if(base.typeof(option) == 'function'){
            option(scripts, text);
        }
        return text;
    };
    ...
    */
     ...
     * @return {String} 
     */
    var vaildBaturuUrl = function(sUrl){
        return (/^(https?:\/\/)?[\w\-.]+\.(51baturu|baturu)\.com($|\/|\\)/i).test(sUrl)||(/^[\w][\w\/\.\-_%]+$/i).test(sUrl)||(/^[\/\\][^\/\\]/i).test(sUrl) ? true : false;
    };
    ...
    var getCharWidth = function(str,fontsize) {
        ...
        d.innerHTML= base.string.encodeHtmlSimple(str);
    ...
    */
    ...
    非法的值
        ...
        http://xxx.qq.com/hi/redirect.html?url=//www.qq.com
    */
    $S.vaildBaturuUrl = vaildBaturuUrl;
